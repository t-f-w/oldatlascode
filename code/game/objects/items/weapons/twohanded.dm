...
/obj/item/weapon/twohanded/chainsaw/attack(mob/target as mob, mob/living/user as mob)
	if(wielded)
		...
		playsound(loc, pick('sound/weapons/chainsawAttack1.ogg', 'sound/weapons/chainsawAttack2.ogg', 'sound/weapons/chainsawAttack3.ogg'), 80, 1, 4)
		if(isrobot(target))
			..()
			return
		if(!isliving(target))
			...
			target.Weaken(4)
			target.emote("scream") //Chainsaw should force screaming due to EXTREME AGONIZING PAIN
			..()
			user.changeNext_move(CLICK_CD_MELEE * 2) //As a balance measure it's not as spammable as other weapons.
		return
	else
		playsound(loc, "swing_hit", 50, 1, -7)
		return ..()
