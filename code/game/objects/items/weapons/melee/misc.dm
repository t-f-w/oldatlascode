...
// if(on)
	...
	if (user.a_intent == "harm" || (extendable && !on))
		...
		if(cooldown <= 0)
			playsound(user, 'sound/weapons/baton1.ogg', 50, 1, -7)
			if(stamina)
...
/obj/item/weapon/melee/combatknife
	...
	can_slit_throat = 1 //You bet your ass it will slit throats.
	w_class = 1.0
	slot_flags = SLOT_BELT
	...
	hitsound_extrarange = -10 //Pretty quiet
...
/obj/item/weapon/melee/concealable/pocketknife/attack_self(mob/living/user)
	...
	if (active)
		...
		can_slit_throat = 0 //What, you expect to slit someone's throat with a stump?
		playsound(user, 'sound/weapons/raise.ogg', 20, 1)
