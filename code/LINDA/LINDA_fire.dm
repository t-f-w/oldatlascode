...
/obj/effect/hotspot
	...
	var/dying = 0 //If this is on it'll cancel all other checks as if it was deleted
...
/obj/effect/hotspot/process()
	...
	if(location.heat_capacity && temperature > location.heat_capacity)
		...
		/*if(prob(25))
			...
			return 0*/
	if(prob(15))
		playsound(src.loc, 'sound/effects/fire.ogg', 30, 1, -7) //play it at low volume so the stacking isn't too bad.
	return 1
...
/obj/effect/hotspot/proc/Kill()
	...
	O.dir = src.dir
	O.loc = src.loc
	//The lighting controller doesn't have enough time to get these running
	// O.luminosity = 2 //Less luminosity than the fire
	// O.l_color = "#FAA460" //Fancy lighting
	animate(O, alpha=0, time=3)
	spawn(3)
		if(O)
			qdel(O)

	if(src) //Check if src still exists, just in case
		qdel(src)
...
/obj/effect/hotspot/Crossed(mob/living/L)
	if(dying) return 0 //No way this can end badly
	..()
	if(isliving(L))
		L.fire_act()
