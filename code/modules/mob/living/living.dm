...
/mob/living/verb/resist()
	...
	if(!L.stat && !L.restrained()) //Restraining someone after headlocking them is GG
		if(L.last_special <= world.time)
			// L.changeNext_move(CLICK_CD_BREAKOUT)
			L.last_special = world.time + CLICK_CD_BREAKOUT //Additional cooldown
			var/resisting = 0
			for(var/obj/O in L.requests) //Would help knowing what this actually is...
				qdel(O)
				resisting++
			for(var/obj/item/weapon/grab/G in usr.grabbed_by)
				resisting++
				if(prob(50) && (G.last_hit_zone == "eyes" || G.last_hit_zone == "mouth")) //Resisting this should always be possible
					L.visible_message("<span class='warning'>[L] manages to push [G.assailant]'s hand from their [G.last_hit_zone]!</span>")
					G.assailant.zone_sel.selecting = "head"
					G.assailant.zone_sel.update_icon()
					L.eye_blind = 1
					L.silent = 1
				if(G.state == GRAB_PASSIVE)
					G.assailant.visible_message("<span class='warning'>[L] has broken free of [G.assailant]'s grip!</span>")
					qdel(G)
					return
				else if(G.state == GRAB_AGGRESSIVE)
					if(prob(40 - (G.force_down * 20))) //20% chance to break free if you're forced down
						if(G.force_down)
							G.assailant.visible_message("<span class='warning'>[L] has broken free of [G.assailant]'s grip, tumbling him down!</span>", \
														"<span class='warning'>You tumble to the ground after [L] resists out of your pindown!</span>")
							G.assailant.Weaken(3)
							step_away(G.assailant,L)
						else
							G.assailant.visible_message("<span class='warning'>[L] has broken free of [G.assailant]'s grip!</span>")
						qdel(G)
						return
					...
					if(prob(5)) //Low as fuck chance
						G.assailant.visible_message("<span class='warning'>[L] has broken free of [G.assailant]'s headlock!</span>", \
													"<span class='warning'>[L] has broken free of your grab, temporarily immobilizing you!</span>") //So nobody complains about "WTF I CAN'T MOVE"
						step_away(G.assailant,L)
						L.stunned = 0 //Instantly remove stun on the victim
						L.update_canmove()
						G.assailant.Stun(2) //Temporarily stun the assailant to give the victim some fighting chance
						qdel(G)
						return
			if(resisting)
				L.visible_message("<span class='warning'>[L] tries to resist!</span>")
				return
		else
			return
