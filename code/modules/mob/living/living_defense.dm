...
/mob/living/proc/pre_bullet_act(var/obj/item/projectile/proj_type, var/def_zone)
	return 1 //Return 0 if you want no messages/etc.
		...
		if(istype(I,/obj/item/weapon)) //If the item is a weapon...
			...
			if (W.throwforce > 0) //If the weapon's throwforce is greater than zero...
				if (W.throwhitsound) //...and throwhitsound is defined...
					playsound(loc, W.throwhitsound, volume, 1, -7) //...play the weapon's throwhitsound.
				else if(W.hitsound) //Otherwise, if the weapon's hitsound is defined...
					playsound(loc, W.hitsound, volume, 1, -7) //...play the weapon's hitsound.
				else if(!W.throwhitsound) //Otherwise, if throwhitsound isn't defined...
					playsound(loc, 'sound/weapons/genhit.ogg',volume, 1, -7) //...play genhit.ogg.

		else if(!I.throwhitsound && I.throwforce > 0) //Otherwise, if the item doesn't have a throwhitsound and has a throwforce greater than zero...
			playsound(loc, 'sound/weapons/genhit.ogg', volume, 1, -7)//...play genhit.ogg
		if(!I.throwforce)// Otherwise, if the item's throwforce is 0...
			playsound(loc, 'sound/weapons/throwtap.ogg', 1, volume, -7)//...play throwtap.ogg.
...
/mob/living/proc/IgniteMob()
	if(fire_stacks > 0 && !on_fire)
		on_fire = 1
		// prev_l_color = l_color
		// l_color = "#FFA500"
		AddLuminosity(3)
		playsound(src, 'sound/Effects/combust.ogg', 40, 1, -7)
		update_fire()
...
/mob/living/proc/grabbedby(mob/living/carbon/user, var/suppress_message = 0)
	...
	// G.synch()
	// G.last_upgrade = world.time //Uncomment this if you want delay for next grab
	LAssailant = user

	playsound(src.loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -7)
...
/mob/living/attack_paw(mob/living/carbon/monkey/M as mob)
	...
	if (M.a_intent == "harm")
		...
		if (prob(75))
			add_logs(M, src, "attacked", admin=0)
			playsound(loc, 'sound/weapons/bite.ogg', 50, 1, -7)
