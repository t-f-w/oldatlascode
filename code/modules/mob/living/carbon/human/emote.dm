/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null, var/special = 0)
		...
		if ("cough")
			if (miming)
				...
				if (!silent)
					...
					playsound(src.loc, sound, 50, 1, -7)
					if(nearcrit)
	...
	////////////////
		...
		if ("ping")
			...
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/machines/ping.ogg', 50, -12)
					message = "<B>[src]</B> pings!"
		if ("buzz")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, -12)
					message = "<B>[src]</B> buzzes."
		if ("buzz2")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/machines/buzz-two.ogg', 50, -12)
					message = "<B>[src]</B> buzzes twice."
		if ("chime")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/machines/chime.ogg', 50, -12)
					message = "<B>[src]></B> chimes."
		if ("honk")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/items/bikehorn.ogg', 50, -12)
					message = "<B>[src]</B> honks!"
		if ("sad")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/misc/sadtrombone.ogg', 50, -12)
					message = "<B>[src]</B> plays a sad trombone."
		if ("warn")
			var/DNA = src.dna.species.id
			switch(DNA)
				if("IPC")
					playsound(src.loc, 'sound/machines/warning-buzzer.ogg', 50, -12)
					message = "<B>[src]</B> blares an alarm!"
		// if ("help") //This breaks default human help emote
		// 	var/DNA = src.dna.species.id
		// 	switch(DNA)
		// 		if("IPC")
...
/mob/living/carbon/proc/fart(var/super = 0)
	...
	spawn(0)
		...
		for(var/i = 1, i <= count, i++)
			...
			if(!B) break
			playsound(src, 'sound/misc/fart.ogg', 50, 1, -7)
			sleep(1)
		B = locate() in src.internal_organs
