...
/mob/living/carbon/human/pre_bullet_act(var/obj/item/projectile/P, var/def_zone)
	var/mob/living/L = check_shields(P.damage, "the [P.name]")
	if(istype(L) && L != src) //Check for bodyshields
		P.Bump(L) //Bump the bullet for messages, etc.
		return 0 //Woop, the bullet hits the body shield. Let's do nothing for src.
	return 1 //Let the projectile code go boom bing bang on src
...
/mob/living/carbon/human/bullet_act(var/obj/item/projectile/P, var/def_zone)
	...
	if(check_shields(P.damage, "the [P.name]") == 1)
	...
	for(var/obj/item/weapon/grab/G in src)
		if(G.assailant == src && G.state >= GRAB_NECK && G.affecting && !G.affecting.lying)
			if(prob(85)) //High chance to hit the body shield instead
				return G.affecting //Special case
...
/mob/living/carbon/human/attacked_by(var/obj/item/I, var/mob/living/user, var/def_zone)
	...
	else //A human that has no DNA!? BLASPHEMY.
		if(user != src)
			user.do_attack_animation(src)
			if(check_shields(I.force, "the [I.name]")) //Ideally this should check for meatshields but humans with no DNA are nonexistant at the moment.
				return 0
