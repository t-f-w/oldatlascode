...
/datum/species/proc/handle_vision(var/mob/living/carbon/human/H)
	if( H.stat == DEAD )
		...
		H.visionoverlay = null //Currently only glasses can give you vision overlay. Subject to change.
		if(H.glasses)
			if(istype(H.glasses, /obj/item/clothing/glasses))
				...
				H.visionoverlay = G.screenoverlay
...
/datum/species/proc/spec_attack_hand(var/mob/living/carbon/human/M, var/mob/living/carbon/human/H)
	if((M != H) && H.check_shields(0, M.name) == 1)
		add_logs(M, H, "attempted to touch")
		H.visible_message("<span class='warning'>[M] attempted to touch [H]!</span>")
		return 0
	var/mob/living/L = H.check_shields(0, M.name)
	if(istype(L) && H != L) //Check_shields returned a mob instead. Let's hurt it!
		add_logs(M, H, "attempted to hit", addition="([L] was hit instead - meatshield)")
		M << "<span class='danger'>You attempted to hit [H] but ended up hitting [L] instead!</span>"
		M.attack_hand(L)
		return 0
	...
	switch(M.a_intent)
		if("help")
			...
			if((H.head && (H.head.flags & HEADCOVERSMOUTH)) || (H.wear_mask && (H.wear_mask.flags & MASKCOVERSMOUTH)))
				...
				return 0
			// if(H.head.has_slit_throat) //Not sure about this.
			// 	M << "<span class='notice'>Their throat is slit... You'll need to insert a tube!</span>" //You need to insert an empty pen to do CPR.
			// 	return 0
		...
		if("harm")
			if (attacker_style && attacker_style.harm_act(M, H))
				...
				if(!damage)
					if(M.dna)
						playsound(H.loc, M.dna.species.miss_sound, 25, 1, -12)
					else
						playsound(H.loc, 'sound/weapons/punchmiss.ogg', 25, 1, -12)
				...
				if(M.dna)
					playsound(H.loc, M.dna.species.attack_sound, 25, 1, -7)
				else
					playsound(H.loc, 'sound/weapons/punch1.ogg', 25, 1, -7)
		...
		if("disarm")
			if (attacker_style && attacker_style.disarm_act(M, H))
				...
				if(randn <= 25)
					...
					playsound(H, 'sound/weapons/thudswoosh.ogg', 50, 1, -7)
				...
				playsound(H, 'sound/weapons/punchmiss.ogg', 25, 1, -12)
...
/datum/species/proc/spec_attacked_by(var/obj/item/I, var/mob/living/user, var/def_zone, var/obj/item/organ/limb/affecting, var/hit_area, var/intent, var/obj/item/organ/limb/target_limb, target_area, var/mob/living/carbon/human/H)
	...
	if((user != H) && H.check_shields(I.force, "the [I.name]") == 1)
		return 0
	var/mob/living/L = H.check_shields(0, "the [I.name]")
	if(istype(L) && H != L) //Check_shields returned a mob instead. Let's hurt it!
		add_logs(user, H, "attempted to hit", addition="([L] was hit instead - meatshield)")
		user << "<span class='danger'>You attempted to hit [H] but ended up hitting [L] instead!</span>"
	...
	if((I.damtype == BRUTE) && I.force)
		if(affecting.brute_dam >= I.bleedcap && prob(I.bleedchance)) //Check items.dm for chances
...
/datum/species/proc/breathe(var/mob/living/carbon/human/H)
	...
	if(H.health <= config.health_threshold_crit)
		H.losebreath++
	var/obj/item/organ/limb/head/U = H.get_organ("head")
	if(U && U.has_slit_throat)
		H.losebreath++ //You suffocate faster with a slit throat
		H.silent += 2 //You are also silenced!
		if(prob(20))
			H.adjustBloodLoss(Clamp(U.bloodloss + 0.01, 0, 1), U) //Increase bloodloss for the head.
			H.emote("cough") // "X makes a strong noise." due to "silent"

	if(H.losebreath>0) //Suffocating so do not take a breath
		H.losebreath--
		if (prob(10)) //10% chance to gasp every tick
			spawn H.emote("gasp")
		if(istype(H.loc, /obj/))
...
/datum/species/proc/handle_breath(datum/gas_mixture/breath, var/mob/living/carbon/human/H)
	...
	if(!breath || (breath.total_moles() == 0))
		...
		if(H.health >= config.health_threshold_crit)
			...
			H.adjustOxyLoss(HUMAN_CRIT_MAX_OXYLOSS) //Something is broken here.. There are cases when you can survive crit forever and your oxyloss is not added.
			H.failed_last_breath = 1
...
/datum/species/proc/IgniteMob(var/mob/living/carbon/human/H)
	if(H.fire_stacks > 0 && !H.on_fire && !(HEATRES in specflags) && !(NOFIRE in specflags))
		...
		H.AddLuminosity(3)
		H.update_fire()
		playsound(H.loc, 'sound/Effects/combust.ogg', 40, 1, -7)
