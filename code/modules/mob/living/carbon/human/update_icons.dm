...
//Latest addition: Made all recolored items properly show up like that on the human. Allows for better washing machine code.
...
/mob/living/carbon/human/update_inv_glasses()
	...
	if(glasses)
		...
		var/image/standing = image("icon"='icons/mob/eyes.dmi', "icon_state"="[glasses.icon_state]", "layer"=-GLASSES_LAYER)
		standing.color = glasses.color
		standing.alpha = glasses.alpha

		overlays_standing[GLASSES_LAYER] = standing

	handle_regular_hud_updates() //Updates the overlays and other updated variables ASAP. Doesn't happen instantly for overlays.
...
/mob/living/carbon/human/update_inv_ears()
	...
	if(ears)
		...
		var/image/standing = image("icon"='icons/mob/ears.dmi', "icon_state"="[ears.icon_state]", "layer"=-EARS_LAYER)
...
/mob/living/carbon/human/update_inv_shoes()
	...
	if(shoes)
		...
		if(feet_blood_DNA)
			...
			overlays_standing[SHOES_LAYER] = null //not sure if this is neccesary
...
/mob/living/carbon/human/update_inv_head()
	...
	if(head)
		...
		standing.color = head.color //This has been done for everything now
...
/mob/living/carbon/human/update_inv_handcuffed() //Added customizability, so cable coils and zipties can finally have unique overlays! ~Crystalwarrior
	...
	if(handcuffed)
		...
		//TODO: Allow recoloring + alpha var similar to previous procs
...
/mob/living/carbon/human/update_inv_r_hand()
	...
	if(r_hand)
		...
		if(!(t_state in icon_states("icons/mob/items_righthand.dmi"))) //TODO should probably populate this list at start?
			standing = image("icon" = 'icons/mob/items_righthand1.dmi', "icon_state" = t_state, "layer"=-L_HAND_LAYER)
		else
			standing = image("icon" = 'icons/mob/items_righthand.dmi', "icon_state" = t_state, "layer"=-L_HAND_LAYER)
...
/mob/living/carbon/human/update_inv_l_hand()
	...
	if(l_hand)
		...
		if(!(t_state in icon_states("icons/mob/items_lefthand.dmi"))) //TODO should probably populate this list at start?
			standing = image("icon" = 'icons/mob/items_lefthand1.dmi', "icon_state" = t_state, "layer"=-L_HAND_LAYER)
		else
			standing = image("icon" = 'icons/mob/items_lefthand.dmi', "icon_state" = t_state, "layer"=-L_HAND_LAYER)
...
/mob/living/carbon/human/proc/wear_female_version(t_color, icon, layer) //Recolored in w_uniform, pretty sure
