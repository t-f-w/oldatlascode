...
/obj/item/weapon/grab/proc/reinforce()
	if(!affecting)
		return
	...
	if(M == affecting)
		if(ishuman(M) && ishuman(assailant))
			...
			switch(assailant.a_intent)
				...
				// 	return
				if("disarm")
					// if(world.time < (last_upgrade + UPGRADE_COOLDOWN)) //LET 'EM PIN!
					// 	return
					if(state < GRAB_AGGRESSIVE)
...
/obj/item/weapon/grab/Del()
	if(affecting)
		...
		if(last_hit_zone == "eyes" || last_hit_zone == "mouth")
			affecting.eye_blind = 1
			affecting.silent = 1
