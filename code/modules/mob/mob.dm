...
/mob/proc/start_pulling(var/atom/movable/AM)
	...
	if (!( AM.anchored ))
		...
		src.pulling = AM
		AM.pulledby = src
		pulling.started_pulling(src) //Callback proc so the object can interact with user when you start pulling it.
		if(ismob(AM))
			var/mob/M = AM
			if(!iscarbon(src))
				M.LAssailant = null
...
/mob/verb/stop_pulling()
	...
	if(pulling)
		...
		pulling.stopped_pulling(src) //Callback proc so the object can interact with user when you stop pulling it.
		pulling = null
