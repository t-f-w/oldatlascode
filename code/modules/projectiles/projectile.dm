...
/obj/item/projectile/Bump(atom/A)
	...
	if(isliving(A))
		var/mob/living/M = A
		if(!M.pre_bullet_act(src)) //Whaaa? Something stopped us from executing the following code! Must be important. (Hint: this is used for meatshield detection)
			return
		var/reagent_note
	...
	if(isturf(A) && hitsound_wall)
		...
		if(suppressed)
			volume = 5
		playsound(loc, hitsound_wall, volume, 1, -7)
