...
/obj/item/weapon/paper/proc/ethanol_act(var/volume)
	if(clearpaper()) //Returned 1? Had stuff. Let's clean the ink away.
		usr << "<span class='notice'>The solution melts away the ink on the paper.</span>"
	else //No text to clean. Make this paper damp.
		usr << "<span class='notice'>You make \the [src] damp with the solution!</span>"
		var/obj/item/damp_paper/O = new(src)
		usr.drop_item(src)
		usr.put_in_hands(O)
		qdel(src)
